...
import type { Expense, ExpenseCategory, SplitType, ExpenseParticipant } from '../types';
...
export function Expenses() {
  ...
  const tripExpenses = expenses.filter(e => e.tripId === activeTripId).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const calculateParticipants = (): ExpenseParticipant[] | null => {
    ...
    if (formData.splitType === 'Equal') {
      ...
      // Handle rounding error on the first person
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!participants) return; // Error is set in calculateParticipants
  ...
  if (tripMembers.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
        ...
        <Link to="/members" className="mt-4 inline-block px-4 py-2 bg-primary-600 text-white rounded-lg">Go to Members</Link>
      </div>
    );
  }
