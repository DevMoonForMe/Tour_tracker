...
export default function AdvancesPage() {
  ...
  const tripContributions = useMemo(() => {
    return [...contributions.filter(c => c.tripId === activeTripId)].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  }, [contributions, activeTripId]);

  const totalAdvances = useMemo(() => tripContributions.reduce((sum, c) => sum + c.amount, 0), [tripContributions]);
  ...
  const advancePerHead = useMemo(() => {
    ...
    const items = Object.entries(sums).map(([memberId, amount]) => {
